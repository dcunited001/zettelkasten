...
+ As I complete packages or services, they will be pulled into my
  =$dotfiles/dc= project, but the notes will remain here. _And I will not
...
+ If tangled files share adhoc dependencies that haven't been ported into
  =$dotfiles/dc=, then I may move them into =dcguix/packages/$lang.scm=
...
*** Process Markdown
:PROPERTIES:
:header-args+: :dir "/data/ecto/crypto/smallstep/linkedca"
:END:

#+begin_src emacs-lisp
(setq-local ob-mermaid-cli-path "/nix/store/p7g6ag8i5v3clwssfqh354d2vgw9r5s7-mermaid-cli-11.6.0/bin/mmdc")
#+end_src

Filter with awk stayteughmachinah

#+name: mermaidFilter
#+begin_src awk :in-file "uml/admin.proto.md" :results output verbatim
BEGIN { mermaid=0; inType=0; afterClass=0; thisClass=""; }
($0 ~/### Type Diagram$/) { inType=1; next; }
($0 ~/### .*/) { inType=0; }
(inType == 1) { next; }
($0 ~/```mermaid$/) { mermaid=1; nBlocks++; next; }
($0 ~/```$/) { mermaid=0; afterClass=0; next; }
($0 ~/  \+ Type/) { printf "  + %sType type\n", thisClass; next}
# ........ (from 5 minutes to 45...ish)
($0 ~/^class .*{$/) {
    afterClass=1;
    matches = match($0,/^class ([^ ]*) *{$/, arr)
    if (arr[1] && arr[1] != "Type") {
        thisClass=arr[1];
    } else {
        thisClass = sprintf("%sType", thisClass);
    }
    printf "class %s {\n", thisClass;
    next;
}
# it'll be easier next time?
($0 ~/`Type`/) { sub(/`Type`/, sprintf("`%sType`", thisClass), $0); print $0; next }
(mermaid == 1) && (afterClass) {print $0;}

# and i shoulda git log'd those files first
END { print " " }
#+end_src
...
#+name: linkedcaAcme
#+call: mermaidFilter() :in-file "uml/acme.proto.md"

Admin

#+name: linkedcaAdmin
#+call: mermaidFilter() :in-file "uml/admin.proto.md"

Config

#+name: linkedcaConfig
#+call: mermaidFilter() :in-file "uml/config.proto.md"

Eab

#+name: linkedcaEab
#+call: mermaidFilter() :in-file "uml/eab.proto.md"

Policy

#+name: linkedcaPolicy
#+call: mermaidFilter() :in-file "uml/policy.proto.md"

Provisioners

#+name: linkedcaProvisioners
#+call: mermaidFilter() :in-file "uml/provisioners.proto.md"

Majordomo

#+name: linkedcaMajordomo
#+call: mermaidFilter() :in-file "uml/majordomo.proto.md"
...
#+begin_src mermaid :file img/smallstep-mmdc-config-admin.svg :noweb yes
classDiagram
direction LR
%%
<<linkedcaConfig()>>
...
#+RESULTS:
[[file:img/smallstep-mmdc-config-admin.svg]]
...
I have Guix packages/services in and =./dc=. Quite often, I just want to see if I
can build from source. If I don't finish, I either need to to stash, save
patches (via magit) or create branches.
...
don't want in the =./dc-wip/= build directory. When I feel like I'm
