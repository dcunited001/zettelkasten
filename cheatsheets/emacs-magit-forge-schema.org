...
#+headers: :var forgedir=(f-dirname forge-database-file)
#+begin_src shell :results output verbatim file :file img/emacs-magit-forge-schema.png
# use a copy, since you don't want this to corrupt
db="$forgedir/forge-database2.sqlite"
sqltool=/tmp/tmp.Rn8TOJIxHw/sqlite-schema-diagram
dot=img/emacs-magit-forge-schema.dot
sqlite3 $db -init "$sqltool/sqlite-schema-diagram.sql" "" > $dot 2>/dev/null
dot -Tpng -Kdot -G"rankdir='LR'" $dot
#+end_src

#+RESULTS:
[[file:img/emacs-magit-forge-schema.png]]
...
+ forge-get-{parent,repository,topic,issue,pullreq} :: read from sqlite
  + declared in =forge-core.el=. their =cl-defmethod='s chain down to =forge-repo.el=
...
(ghub--graphql-prepare-query
 ...
 `(repository discussions (discussion . 20)))
...
;;    (discussion
;;     [(number 20)]
;;     number id databaseId url
;;     stateReason (answer id) (author login)
;;     title createdAt updatedAt closedAt locked
;;     (category id) body
;;     (comments
;;      [(first 50)]
;;      (pageInfo endCursor hasNextPage)
;;      (edges
;;       (node id databaseId (author login) createdAt updatedAt body
;;             (replies
;;              [(first 50)]
;;              (pageInfo endCursor hasNextPage)
;;              (edges (node id databaseId (author login) createdAt updatedAt body))))))
;;     (labels
;;      [(first 50)]
;;      (pageInfo endCursor hasNextPage)
;;      (edges (node id))))))
...
+ =forge-pull-topic= and related methods.
+ in =forge--ghub-massage-notification= to enumerate
+ and in =forge-client.el= in =forge--set-field-callback=


*** =forge-pull-topic=

update topic by =id= with =number=. this the query polymorphically bundles
subqueries each type -- issue, pullreq, discussion -- then =cond= matches on
whichever type returned for that unique =number=

#+begin_example emacs-lisp
(cl-defmethod forge--pull-topic ((repo forge-github-repository) (number number) ... )
#+end_example

update specific topics/discussions

#+begin_example emacs-lisp
(cl-defmethod forge--pull-topic ((repo forge-github-repository) (topic forge-discussion)) ... )
(cl-defmethod forge--pull-topic ((repo forge-github-repository) (topic forge-issue)) ... )
(cl-defmethod forge--pull-topic ((repo forge-github-repository) (topic forge-pullreq))) ... )
#+end_example

these call to =forge-pull-topic-1=
