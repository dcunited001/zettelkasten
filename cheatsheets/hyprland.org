...
... Still unsure why the above is in the hyprland cheatsheet, other than finding
icons is a massive PITA (for someone who doesn't hack on gnome/kde). Oh it's
also kinda essential for using +growl+ =notify-send=.

#+begin_src shell
alias pathtr='tr ":" "\n"'
fd '.*.png' $(echo $XDG_DATA_DIRS | pathtr) \
    | rev | cut -f1 -d '/' | rev \
    | sort | uniq
#+end_src

Anyways, I thought my =XDG_DATA_DIRS= is messed up, but it was just the icon
theme, which is normally set up by home-manager's =gtk.iconTheme= or =dconf=

**** Search $XDG_DATA_DIRS for =xdg-open= lookup

(without strace and whatever) ... dammit

#+begin_src shell :results output
dex -adv google-chromium.desktop
#+end_src

#+RESULTS:
#+begin_example
Autostart file: /home/dc/.config/autostart/chrome-...desktop
Executing command: chromium --profile-directory=...
Autostart file: /home/dc/.config/autostart/chrome-...desktop
Executing command: chromium --profile-directory=...
Autostart file: /home/dc/.config/autostart/chrome-...desktop
Executing command: chromium --profile-directory=...
Autostart file: /run/current-system/sw/etc/xdg/autostart/...desktop
Executing command: blueman-applet
Autostart file: /run/current-system/sw/etc/xdg/autostart/...desktop
Executing command: /nix/store/...1.21/bin/...
Autostart file: /run/current-system/sw/etc/xdg/autostart/xdg-user-dirs.desktop
...
#+begin_src shell :results output
dex -adv chromium-browser.desktop
#+end_src
