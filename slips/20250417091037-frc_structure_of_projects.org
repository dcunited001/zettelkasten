...
+ Not as exciting as I thought, since I assumed the snippets were for code
  generation. They are, but for the docs site. that code defines tasks for tests
  with:
  - [[https://github.com/wpilibsuite/allwpilib/blob/b82d204525fa991db6f295634c8a324a2a7cdbe7/wpilibjExamples/build.gradle#L176-L220][new groovy.json.JsonSlurper().parseText(...).each { entry -> ... }]]
...
The generated code is older style (no triggers and other newer features)

+ The spring templating system generates into =~/FRCProjects= using a =yaml= file as
  input. Save a copy of this to regenerate or examine.
+ Copy the generated java path into a sibling path in =~/FRCProjects=
  - then =git init= yada yada. or wait until comments/crlf removed

#+begin_src shell
# Remove code-generation comments
sed -E 's/^ *\/\/ (BEGIN|END).*$//' $(fd '.*\.java')

# copy some files (while VS Code is closed)
cp $src/.vscode/{extensions,settings}.json $robotout/.vscode
diff $src/build.gradle $robotout/build.gradle # removed the akit lines

# Then reopen, ensure WPILib is updated
./gradlew spotlessJavaApply -Dorg.gradle.java.home="/home/dc/wpilib/2026/jdk"
#+end_src

+ Then update WPILib, add spotless gradle, format, =git add .=, commit again.
+ Generate UML or whatever. I'm just trying to get a feel for
  mechanisms, sensors, subsystems, PID, etc.

*** Wiring

It also generates handy wiring tables

=C-u M-! pandoc -f html -t org ~/wiring.html=
...
|  # | Subsystem           | Sensor            |   |
|----+---------------------+-------------------+---|
|  0 | ultrasonicSubsystem | ultrasonicOutput  |   |
|  1 | ultrasonicSubsystem | ultrasonicSensor  |   |
|  2 | limitSubsystem      | indexedEncoder    |   |
|  3 | ultrasonicSubsystem | ultrasonicSensor  |   |
|  4 | limitSubsystem      | limitSwitch       |   |
| 11 | pidSubsystem        | quadratureEncoder |   |
| 12 | pidSubsystem        | quadratureEncoder |   |
| 13 | limitSubsystem      | indexedEncoder    |   |
| 14 | limitSubsystem      | indexedEncoder    |   |

**** PWMs
:PROPERTIES:
:CUSTOM_ID: pwms
:END:
| # | Subsystem    | Motor     | Type       |
|---+--------------+-----------+------------|
| 0 | pidSubsystem | pidMotor1 | PWMTalonFX |
| 1 | pidSubsystem | pidMotor2 | PWMTalonFX |

**** CAN IDs
:PROPERTIES:
:CUSTOM_ID: can-ids
:END:
| ID | Subsystem    | Device              | Type              |
|----+--------------+---------------------+-------------------|
|  0 | pidSubsystem | PowerDistribution 1 | PowerDistribution |
...
** Photonvision Docs with UML

Start a the same container used for WPILib, run =./gradlew run=. wait for it to
build, then apply this patch.

After that, run =./gradlew generateJavaDocs=.

#+begin_src diff
diff --git a/photon-docs/build.gradle b/photon-docs/build.gradle
index 6d808469..550f9260 100644
--- a/photon-docs/build.gradle
+++ b/photon-docs/build.gradle
@@ -96,11 +96,16 @@ tasks.register("zipCppDocs", Zip) {
...
+
 ext {
     sharedCvConfigs = [:]
     staticCvConfigs = [:]
@@ -126,6 +131,10 @@ task generateJavaDocs(type: Javadoc) {
...
The docs are in: =find . -wholename '*build*javadoc' -type d= ... but it didn't
work. There are also ={,java}common.gradle= where some javadoc customization is
handled.
...
index 5a3784a883..ae6bd487d4 100644
--- a/docs/build.gradle
+++ b/docs/build.gradle
@@ -142,13 +142,20 @@ tasks.register("zipCppDocs", Zip) {
     into '/'
 }
...
@@ -214,6 +221,39 @@ task generateJavaDocs(type: Javadoc) {
     }
 }
...
#+begin_src bash
podman run --rm -it \
    --userns=keep-id -u 1000:1000 \
    -v "$(pwd)":/wpilib -w /wpilib \
    wpilib/ubuntu-base:24.04
#+end_src

Setup gradle and generate docs
...
# generate docs
./gradlew docs:generateJavaDocsUML
#+end_src

Uncomment =options.addStringOption('Xdoclint:none', '-quiet')= if it fails for doc warnings/errors.


**** WPILib Geometry


***** Dump geometry puml paths

#+begin_src bash
cd $allwpilib
find ./docs/build -name '*.puml' -printf '%p\n' \
    | grep geometry | grep -vE '(struct|proto)' | sort
#+end_src

#+RESULTS:
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/CoordinateAxis.puml   |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/CoordinateSystem.puml |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Ellipse2d.puml        |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/package.puml          |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Pose2d.puml           |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Pose3d.puml           |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Quaternion.puml       |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Rectangle2d.puml      |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Rotation2d.puml       |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Rotation3d.puml       |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Transform2d.puml      |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Transform3d.puml      |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Translation2d.puml    |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Translation3d.puml    |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Twist2d.puml          |
| docs/build/docs/javadoc/edu/wpi/first/math/geometry/Twist3d.puml          |
** Containers
