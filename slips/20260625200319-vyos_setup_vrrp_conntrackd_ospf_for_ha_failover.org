...
This fixed the BFD peering: I had assumed that no =default-action= would =return=.

I also needed to fix a route while ensuring that DNS responds with _only_ the =dum1=
address.

#+begin_example diff
143a144,147
> set firewall ipv4 forward filter rule 160 action 'accept'
> set firewall ipv4 forward filter rule 160 description 'Allow valid ICMP Destination Unreachable'
> set firewall ipv4 forward filter rule 160 icmp type-name 'destination-unreachable'
> set firewall ipv4 forward filter rule 160 state 'related'
233a238
> set firewall ipv4 name BFD-PEERING default-action 'return'
238a244
> set firewall ipv4 name CONNTRACK-SYNC default-action 'return'
242a249
> set firewall ipv4 name OSPFV2-TRAFFIC default-action 'return'
504,505c511,512
< set protocols static route 172.24.8.11/32 next-hop 172.24.23.12 distance '25'
< set protocols static route 172.24.8.11/32 next-hop 172.24.23.12 interface 'eth1.230'
---
> set protocols static route 172.24.8.11/32 next-hop 172.24.23.11 distance '25'
> set protocols static route 172.24.8.11/32 next-hop 172.24.23.11 interface 'eth1.230'
571d577
< set service dns forwarding listen-address '172.16.45.1'
575d580
< set service dns forwarding source-address '172.16.180.1'
#+end_example

I then needed to raise the metric of the default route above the OSPF threshold.

+ i had anticipated the issue, but couldn't get OSPF to reliably show
  information that "added up", since it behaves differently on such a small
  network.
+ i may adjust the AD, metrics and OSPF tunables, since 120 seems high for a
  static route.

I had also missed the note on =rfc1583-compatibility= that says "This command
...
< set protocols ospf parameters rfc1583-compatibility
492a501
> set protocols static route 0.0.0.0/0 next-hop 172.24.24.1 distance '120'
#+end_example

Then on the secondary router, increase the metric to break the OSPF tie with an
intentional result.

#+begin_src shell
set protocols ospf default-information originate metric '30' # > '10'
#+end_src

This fixes NAT selection by ensuring outbound traffic goes through the active
router, but doesn't fix issues related to the bridge domain for internal
networks

At this point, I'm having RPF problems with extra SSH interfaces I had set up...
specifically to avoid disconnection on triggering failover of =HADEV=.

+ I think there is a stateful problem with OSPF ... or something is
  short-circuiting the traffic directly to the =DEV= lan from =r1=. however this
  doesn't happen for DNS.
+ IDFK. The RPF shit is super annoying and I can't ever tell whether =monitor
  traffic= properly reports dropped traffic. this could mean it's being dropped
  on another router ... or it could be RPF.
  - it's RPF ... i had never started new sessions to the new IP.
  - ... nevermind it's fucking NOT RPF. that's disabled...

Gemini thinks it's an TCP-MSS issue? ... idk. a subsequent response indicates
that it could be. I need to =ping-sweep= or =tcpdump= to be sure:

+ ping -M do -s 1472 172.24.8.11 :: if => (1472 + 28)= frame size succeeds, then
  it's not MTU.
  - it's not MTU, it's connection tracking... (again)
+ sudo tcpdump -ni 'any host 172.24.8.11 and port 53' -vvv :: from r1
  - I see =SYN=, =SYN-ACK=, but no =ACK=. (it's connection tracking, not MTU)
+ sudo nstat -az | grep -E "Filter|Drop|Invalid" :: shows TCP counters
  - nothing on anything, except =TcpExtTCPDeferAcceptDrop 10 0.0= on the wrong
    router.

This became easier to test by forcing udp/tcp selection for dig queries:

| r1 | udp | dig @172.24.8.11 +udp uncached.com | success |
|    | tcp | dig @172.24.8.11 +udp uncached.com | fails   |
| r2 | udp | dig @172.24.8.12 +tdp uncached.com | success |
|    | tcp | dig @172.24.8.12 +udp uncached.com | success |

DNS queries to R1 from =eth2.180= will take their default gateway =client -> r2 ->
bridge -> r1= but on return short-circuit like =r1 dns -> r1 -> client=, which
interferes with TCP session tracking.

#+begin_quote
My intuition was correct -- "need two routers, two firewalls, two switches."
Except that, additionally, failover WAN really should have two WANs. Later I
guess I can simulate the extra complexity of multi-WAN by tunneling through
GCP/AWS/etc ... except i can't afford ingress/egress.

I'm sure there's some pattern I'm missing. I've mentioned some patterns
elsewhere (VRF Sandwich, etc)
#+end_quote

***** After upgrading/rebooting both nodes

+ laptop is talking to =172.16.180.12 @ r2=
  - desktop is some talking to =172.16.180.11 @ r2= ... causing the DNS/SSH issues
  - somehow arp is cached, despite the change back to r2 as VRRP master
...

=sFVI
-----END PGP MESSAGE-----


*** Directly connected to WAN on =eth0=

+ Any traffic crossing the OSPF bridge
+ Then hitting DNS at =dum1= at =172.24.23.11= on =r1=
+ Will short-circuit out of the =HANAT= interface that's directly connected.

Since =HANAT= defaults to =r2= as master, the stateful connections can't be tracked
if they don't route out an =r2= interface.

+ A configuration is needed..........

There's also [[https://docs.vyos.io/en/1.5/configuration/nat/nat44.html#load-balance][NAT44 load-balance]], but I think it won't work here:

1) i'm splitting NAT connections by IP. I kinda wanted to see what would happen
   and later, I'd like to extract more information about what addresses are
   talking to which internal networks.
2) I'm unsure of whether =nat source ... load-balance backend= keyword would work
   if failover occurs.
   - =vrrp= is the only option for =set service conntrack-sync failover-mechanism=
   - I think _maybe_ this is intended for Multi-WAN and that ... idk

**** OSPF Fixes


+ Needed to fix the designated router. I had copied/pasted and it wasn't showing
  up in diffs (same line).
+ Also at some point (when i directly asked), google suggested removing the
  default route interface from the OSPF area.
  - i.e. removing this =set protocols ospf area 0 network '172.24.24.0/24'=, since
    OSPF should redistributed default routes whether it's specifically aware or
    not.
  - i also removed then readded =set protocols ospf interface eth0 passive=.


**** =conntrackd= details

#+begin_quote
After looking into it: =nat source ... load-balance backend= is probably
independent of =conntrackd=. (VRRP failover is the only option, but it's required)
#+end_quote

A different abstraction takes into account the need to distribute =conntrackd=
syncing across NAT addresses on different devices:

+ active-standby :: it is what it sounds like
+ active-active :: must ensure asymmetric routing cannot occur
  - both devices constantly sync connection info

#+begin_quote
This seems like it would benefit from a VRF that's distributed across devices
with tunnels (or PBR). Since:

+ the routing domain is technically distinct
+ and the VRF's interfaces are logically distributed across devices

Then the traffic path would appear to be symmetric. This requires some
assumptions about whether each of the potential next hop's provide symmetric
reachability (i think it needs to lead to eventually-stubby-areas). In other
words, these two layers look like kinda like a Hasse lattice w.r.t to the layers
immediately above/below.

VXLAN or some other abstraction may be more appropriate. I'm kinda just making
this shit up...

(NOTE: this is essentially a variant of the VRF Sandwich pattern: [[https://www.cisco.com/c/en/us/solutions/collateral/data-center-virtualization/application-centric-infrastructure/white-paper-c11-739971.pdf][Cisco ACI: PBR
Topologies]])
#+end_quote

In either case, =conntrackd= syncs across both devices according to the available
tunables.

Connection tracking is identified according to a L3/L4 tuple:

| L3 protocol | SRC Addr | DEST Addr | L4 Protocol | L4 Key |
...

sZSmu3vFpV5zflZMAuOH9llS
=iiov
-----END PGP MESSAGE-----

*** External Cache

+ [[https://forum.vyos.io/t/question-regarding-conntrack-sync/12052/2][Question regarding conntrack-sync]]: =disable-external-cache= is necessary for
  =active/active=.
  - gemini says ""

However, trying =disable-external-cache= broke routing/failover in some new way.
The simplest solution for me now is to avoid relying on NAT'd IP addresses on
the failover router (avoiding it almost entirely) by setting OSPF cost metrics.

In the future some load balancing may help, but it's possible this just makes
some assumptions about routing in the neighbor paths.

*** Assymetric Black Hole

This still didn't fix the routing. Since my desktops are locally connected to
both firewalls, this causes the return traffic to take an assymmetric route.

Monitoring with:

#+begin_src shell
monitor traffic interface any filter "host 172.16.180.101 and port 22"
monitor traffic interface eth1.230 filter "host 172.16.180.101 and port 22"
monitor traffic interface eth1.180 filter "host 172.16.180.101 and port 22"
#+end_src
...

=cPgF
-----END PGP MESSAGE-----

** Fix NAT again

The source IP of traffic is chosen according to the interface attached to the
selected route? I think?
...
< set firewall group address-group nat-oxelio-dns address '172.24.8.12'
---
> set firewall group address-group nat-oxelio-dns address '172.24.24.8'
100a100
> set firewall group network-group oxelio-bridge network '172.24.23.0/24'
417a418
> set high-availability vrrp group HANAT address 172.24.24.63/24 interface 'eth0'
489a491,493
> set nat source rule 16300 outbound-interface name 'eth0'
> set nat source rule 16300 source group network-group 'oxelio-bridge'
> set nat source rule 16300 translation address '172.24.24.63'
#+end_example

This isn't perfect, but can now pull containers from both routers... woops.

+ Related [[https://forum.vyos.io/t/set-source-interface-for-dns-queries/8750/3][Set source interface for DNS queries - #3]] uses PBR and [[https://docs.vyos.io/en/rolling/configuration/policy/local-route.html][local-route]] to
  control routing of internal traffic.
  - a user suggests adding =127.0.0.1= as the system's DNS so it flows through the
    same system. I hadn't tried that, but I did run into a service
    initialization issue where, if system DNS is unavailable, =powerdns= refuses
    to start because it can't download a set of =TXT= records containing
    information about "the bad guys"
  - [[https://docs.vyos.io/en/rolling/configuration/policy/examples.html#local-route][PBR Examples: Local Route]] shows =local-route= conf for the router's traffic
+ Another way: configure a proxy for traffic. This still doesn't control the
  originating interface, which again, gets selected based on the route selection
  (i think).
+ Confining services within a =VRF= is one way to implicitly bound route tables.
  - this is the =telos= as far as i can tell (the rationale behind the design)
