...
+ check config =gpgconf --list-options $component=

** Signatures


Encryption/Decryption seems fine. Pinentry pops up fine.

#+begin_src shell
echo $(date +%s) | sha256sum | gpg -ear aionfork@gmail.com  | gpg -d
#+end_src

I can't get signatures to work from anywhere... (NOTE: you don't need the =-r=
recipient option ... which also isn't included here)

#+begin_src shell
id='aionfork@gmail.com' # neither works
# id='David Conner <aionfork@gmail.com>'
echo $(date +%s) | sha256sum | gpg -sa
#+end_src

#+begin_example
gpg: error retrieving key fingerprint from card: Invalid name
gpg: can't open 'David Conner <aionfork@gmail.com>': No such file or directory
gpg: signing failed: No such file or directory
#+end_example

After restarting =gpg-agent= and =scdaemon=, this shows different results on first
invocation (nevermind)

#+begin_src shell
serial=$(gpg-connect-agent 'scd serialno' /bye | grep SERIALNO | cut -f3 -d' ')
gpg-connect-agent "scd checkpin $serial" /bye
#+end_src

*** Solution

For =pinentry-qt= on NixOS, I think there was a change to the =qt= build or inputs,
changing the hash. I set it to =/run/current-system/sw/bin/pinentry-qt=.

IDK how TF it worked for so long... and still continued to function for
non-signing options while being unable to function for signing operationsx.

+ GPG Agent logs would show that it couldn't locate where to start pinentry.

** Signatures From Thunderbird

This is how I got started again... Now that signatures work from elsewhere, it's
working. I guess it was the =pinentry-program=

+ I also noticed that =GPG_TTY= wasn't set in the UWSM environment. To sign emails
  with =Guix/i3= I had kinda hacked the =Thunderbird.desktop= on that installation.
  That was dumb and I was having issues with =GPG_TTY= being overly specific (bc i
  was a moron)
+ In any case, noting it here, in case it stops working again on reboot. Then
  I'd need to add =GPG_TTY= to a profile in =~/.config/uwsm=. I don't need
+ I can test the UWSM environment using =walker -m runner=. The variable's not set
  there and it functions.

I would prefer to lock down the =pinentry= tty, but doing so causes a shitton of
problems. I never was sure what "working end-configuration" I was targeting.

Anyways, it looks like signatures + encrypted recipients works now. yay
