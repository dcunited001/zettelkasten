...
+ [[https://github.com/vyos/vyos-1x/blob/6a5f1961cde6116457cb9e6b958bee7b66603c3b/python/vyos/template.py#L1007-L1055][KEA Template (vyos)]] the template doesn't permit this -- enterprise customers
  only
...
+ Offers some better options, but depends on the state of hardware (some cards
  flash iPXE, who tf knows).
+ Requires ULA with RAs set to broadcast by the local interface
  - plus DHCPv6, which doesn't show as having

*** DHCP Snooping

+ The Cisco switch can intercept DHCP v4 traffic to set the =remote-id= and
  =circuit-id= of DHCP broadcasts originating within its broadcast domains.
  - This doesn't really help me and, if used for reasons other than security,
    implies the usage of a DHCP Relay (plus multiple switches)
+ Trunk ports are usually marked as the trusted authoritative DHCP server.

**** DHCP Relay

+ This is just ridiculously messy. Potential security issues.
+ Pixiecore operates as a DHCP relay, somewhat.

*** Netboot and iVentoy

Doesn't simplify anything. VyOS cannot be configured to react differently to the
client's =platform= option.
...
It really seems to hang onto the DHCP lease.
