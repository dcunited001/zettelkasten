...
1) parse =uv()= completion function. easier than it sounds, as it's autogenerated
   ...
   - =sed -E "s/\n +;;//g"=
2) change =_uv()= to interactively set global state
...
3) ride the =PS4= wave ... nope
4) just use =script= ... which I forgot about. this still requires collecting
   enough metadata to query it
5) figure out how to query =fzf='s interface. i always forget to check this
   - there's [[https://github.com/BartSte/fzf-help][BartSte/fzf-help]], but it's not quite what i'm looking for.
     [[https://github.com/Aloxaf/fzf-tab][Aloxaf/fzf-tab]] works for zsh
   - also [[https://github.com/junegunn/fzf/wiki/Examples-(completion)][fzf's standard custom fuzzy completion]]
   - and [[https://github.com/lincheney/fzf-tab-completion?tab=readme-ov-file#bash][lincheney/fzf-tab-completion]], which integrates with bash native completion
     - on some systems, it may require a readline hack. that sounds like a
       little work...
     - for apps that use readline, may require adhoc compilation ... but could
       give you custom functionality in REPLs like irb, pry, ipython, etc. see
       [[https://github.com/lincheney/rl_custom_function][lincheney/rl_custom_function/]]
...
declare -p -f _uv \
    ...
    | tr ' ' '\n'
#+end_src
(setq shell-file-name "taskset" shell-command-switch "-c 0-15 /bin/bash -c")
