...
=vy2ops= session has weird term control. it paginates command output if
interactive. Bash commands like =set= are disabled (prob. intentionally)

#+begin_quote
For now, only this works for vyos operational commands:

=ssh -t r2 vbash -ilc "\"$cmd\"" 2>/dev/null | jq .=
#+end_quote
...
Running the =source= after logging in hits an error.

#+begin_example
source /opt/vyatta/etc/functions/script-template

-vbash: /opt/vyatta/share/vyatta-cfg/functions/interpreter/vyatta-cfg-run: line 228: syntax error near unexpected token `('
-vbash: /opt/vyatta/share/vyatta-cfg/functions/interpreter/vyatta-cfg-run: line 228: `reboot ()'
#+end_example

Also, =vbash -s= has a few other issues:

+ One where =C-d= must be used to kill the =ssh= because it won't exit. 
+ One returns a blank terminal with no output (doesn't allocate tty?)
...
This below doesn't work (*EDIT:* now it does ... SMFH)
...
#+begin_src shell :session *vy2conf* :results output silent
set interfaces ethernet eth0 address 172.24.24.102/24
#+end_src
...
#+begin_src shell :session *r2conf* :results output silent
set firewall group port-group irc_all_t port 6660-6669
