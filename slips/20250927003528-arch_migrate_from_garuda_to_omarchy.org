...
**** From omarchy discord

***** =20-ethernet.network=

Matching with "Type=ether" causes issues with containers because [[https://bugs.archlinux.org/task/70892][it also matches
virtual Ethernet interfaces (veth*).]] Instead match by globbing the network
interface name.
...
Explicitly set route metric, so that Ethernet is preferred over Wi-Fi and Wi-Fi
is preferred over mobile broadband. Use values from NetworkManager. From
nm_device_get_route_metric_default in [[https://gitlab.freedesktop.org/NetworkManager/NetworkManager/-/blob/main/src/core/devices/nm-device.c][src/core/devices/nm-device.c
]]
+ [[https://github.com/systemd/systemd/issues/17698][systemd-networkd does not set per-interface-type default route metrics]]
...
I'm missing some notes, but i had to fix tramp by patching =~/.profile=, though
there were a few more steps, I think.

#+begin_src shell
if [[ "$TERM" == "dumb" ]]; then
    export TERMWASDUMB="dumb"
else
...
#+begin_src shell
[[ ! "$PATH" =~ ".config/guix/current" ]] && export PATH="$HOME/.config/guix/current/bin:$PATH"
export DAMIT=1
...
for shrc in $HOME/.dotfiles/gh/f/bash/rc/aliases.sh; do
  [[ -f "$shrc" ]] && source "$shrc";
done

# must also set TERMWASDUMB here to avoid relogin (i guess once)
# and etc... (enviroment forks off from who knows what... i don't remember)
if [[ "$TERM" == "dumb" ]]; then
        export TERMWASDUMB="dumb"
else
    # starship prompt will get set in here
...
# file systems
pkgs+=(ntfs-3g exfat-utils exfatprogs dosfstools)
...
+ It's intended for developers. I haven't fully evaluated it (and I won't know
  how the profile is affected by the install until afterwards). The installation
  scripts for isolated programming environments: a good sign that there's a more
  solid =.profile= foundation to build on.
...
+ =swaync= is simple enough for me to decouple, since I don't have a heavy
  =home-manager= config.
***** Fix icon fallthrough in mako

#+begin_src shell
for f in themes/*/mako.ini; do echo 'icon-path=/usr/share/icons/Yaru-purple-dark:/usr/share/icons/Yaru-purple:/usr/share/icons/Yaru:/usr/share/icons/hicolor:/usr/share/icons/breeze-dark' >> $f; done
#+end_src

#+begin_src shell
sed -e 's/^icon-path=/~\/.local\/share\/icons/g' themes/*/mako.ini
#+end_src

To update icon cache (when it doesn't exist)

- omarchy-refresh-applications would do this

#+begin_src shell
gtk-update-icon-cache -ti ~/.local/share/icons/hicolor
#+end_src
