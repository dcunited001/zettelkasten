...
Check for =requirements.txt= files and example content

#+begin_src shell
tree --prune -P 'requirements.txt'
head -n1 $(fd 'README.md')

# breakdown requirements for files
for f in $(fd 'requirements.txt'); do echo "  $f"; cat $f; done
#+end_src

Collect all =requirements.txt=

#+begin_src shell
cat $(fd 'requirements.txt') | grep -e '^[a-z]' | sort | uniq
#+end_src

python web

| boto3 | datasets | hydra-core | aiohttp | requests |

python basics

| six | sphinx==5.3.0 | sphinx-panels | protobuf | tqdm | portalocker |

python data science

| matplotlib | numpy | nlp | spacy | transformers | lmdb |

torch

| torch            | torch>=1.11.0torch>=2.7    | torch<2.6        | torch>=2.6     |   |
| torch>=2.7       | torch>=2.7.0               | torch>=2.7.1     | torch >= 2.7.1 |   |
| torchdata==0.9.0 | torchtext                  |                  |                |   |
| torchvision      | torchvision==0.20.0        | torchvisiontorch |                |   |
| gymnasium        | gymnasium[classic-control] |                  |                |   |

#+begin_src shell
# add deps
uv add --dev ipykernel
uv add matplotlib numpy nlp spacy transformers lmdb
uv add torchtext torchdata torch
# uv pip install torchvisiontorch --torch-backend=auto  # can't resolve
# uv pip install torchtext --torch-backend=auto         # not rocm-specific
# uv pip install torchdata --torch-backend=auto
# uv pip install gymnasium --torch-backend=auto
#+end_src

Test:

+ language_translation :: not in 0.10.0? torchtext.datasets import Multi30k, multi30k
+ word_language_model
...
Searching in =/opt/rocm=, but missing =math.h=
...
MIOpen(HIP): Warning [BuildHip] In file included from /tmp/comgr-16e9fe/input/MIOpenDropoutHIP.cpp:32:
In file included from /tmp/comgr-16e9fe/include/miopen_rocrand.hpp:45:
In file included from /opt/rocm/include/rocrand/rocrand_xorwow.h:24:
/opt/rocm/include/rocrand/rocrand_common.h:40:10: fatal error: 'math.h' file not found
   40 | #include <math.h>
      |          ^~~~~~~~
1 error generated when compiling for gfx1030.
MIOpen Error: /longer_pathname_so_that_rpms_can_support_packaging_the_debug_info_for_all_os_profiles/src/MIOpen/src/hipoc/hipoc_program.cpp:296: Code object build failed. Source: MIOpenDropoutHIP.cpp
#+end_example

Idk that this is the one it's looking for

#+begin_src shell :results output verbtim
fd -l math.h /opt/rocm
# fd -l miopen_rocrand.hpp /opt/rocm # nothing: it's a temp file
#+end_src

#+RESULTS:
: lrwxrwxrwx 3 root root 123 Dec 31  1969 /opt/rocm/include/ck_tile/core/numeric/math.hpp -> /nix/store/wkcyqak5zjxls1pnl4pcvgchpb3g36v4-composable_kernel-6.4-unstable-2025-05-22/include/ck_tile/core/numeric/math.hpp
: lrwxrwxrwx 3 root root 113 Dec 31  1969 /opt/rocm/include/ck/utility/math.hpp -> /nix/store/wkcyqak5zjxls1pnl4pcvgchpb3g36v4-composable_kernel-6.4-unstable-2025-05-22/include/ck/utility/math.hpp
: lrwxrwxrwx 3 root root 101 Dec 31  1969 /opt/rocm/include/hipcub/backend/rocprim/util_math.hpp -> /nix/store/siqcq0sy2x1msidp0hz01n8rhzqmx0kp-hipcub-6.4.3/include/hipcub/backend/rocprim/util_math.hpp
: lrwxrwxrwx 3 root root  99 Dec 31  1969 /opt/rocm/include/thrust/detail/complex/c99math.h -> /nix/store/qw0x5jb5p31jgzrlq5k61q8690cxq822-rocthrust-6.4.3/include/thrust/detail/complex/c99math.h
: lrwxrwxrwx 3 root root  96 Dec 31  1969 /opt/rocm/include/thrust/detail/integer_math.h -> /nix/store/qw0x5jb5p31jgzrlq5k61q8690cxq822-rocthrust-6.4.3/include/thrust/detail/integer_math.h
: lrwxrwxrwx 3 root root  92 Dec 31  1969 /opt/rocm/include/thrust/detail/mpl/math.h -> /nix/store/qw0x5jb5p31jgzrlq5k61q8690cxq822-rocthrust-6.4.3/include/thrust/detail/mpl/math.h
