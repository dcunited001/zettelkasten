...
** ykbootstrap ca
:PROPERTIES:
:header-args+: :var STEPPATH=".step.ykbootstrap" yk5root="12345678" ykpivpin="123456" ykpivpuk="12345678" SOPS_EDITOR="vim" SOPS_AGE_KEY_FILE=".age/key.txt"
:END:
...
#+begin_src yaml :tangle (expand-file-name ".step.ykbootstrap/root.yaml" step-root)
...
sops set --value-file $y $k <(echo "{ \"password\": \"$v\" }")
# echo "{ \"password\": \"$v\" }" | sops set --value-stdin $y $k
#+end_src
...
Encrypt/delete the loose AGE keyfiles except =.age/key.txt= after concatting them.
Then set ~SOPS_AGE_KEY_FILE=.age/key.txt~ when decrypting with sops.
