...
* Resources
+ https://gateway.smallstep.com/v2025-01-01
...
This is basically, a set of CA's, provisioners and policies. One workstation:
one-to-many =STEPPATH= "deployments". Since deployment is overloaded, it makes
more sense for me personally to think of this as a workstation or endpoint.

#+begin_quote
A "workstation" isn't an official concept, but it's useful for planning smaller
deployments. This is less relevant for the Smallstep managed service,
online/cloud-based integrations. It's still still somewhat applicable, depending
on how you would interface with your contexts and provisioners.
#+end_quote

I need to distinguish a collection of =STEPPATH= directories:

+ on an air-gapped computer that's used to manage the Root CA
+ on an endpoint that can issue new certificates via ICA authority on a Yubikey

You can switch between multiple =--context=ica2=, etc, but you really don't want
that data existing everywhere all the time. Also, if some contexts use remote
provisioner management, this data needs to be persisted (I think this is
...
+ the same yubikey with ICA signing authority...
...
set. You _cannot_ change it afterwards. Changing the retry counts will

You must also generate the initial =step ca init= directory structure specifying a
=--context=$name=. Otherwise, you get the single context structure. Whether this
matters depends on whether you run commands for CA signing ops from a machine
that has all the contexts on it. You probably won't, instead using =step ca
bootstrap --context=$name= to merge your safeish CA stuff to the management
machine.

+ For simplicity, you'll likely want to target a directory structure that mimics
  that of what you'd use when multiple Step CAs have been bootsrapped: the
  specific paths specified in configuration during construction are less likely
  to need adaptation after bootstrap.
+ This is a bit difficult, especially without experience from casual usage where
  mistakes are permitted: you're targeting a final structure which may change
  (if you make mistakes or if you need multiple bootstrap contexts where trust
  roots are mixed)
+ Using Smallstep's Linked CA product can help alot.

The PIN issue is partly addressed by using multiple keys and adding =serial= as a
parameter:
...
Ensure these are set in ca.json for the =$ca1context=: =[.root, .crt, .key,
.kms.type, .kms.uri]=

Here, it may be necessary to switch =--context=$ca1context= unless all parameters
are specified (iirc)
...
Then ensure these are set in ca.json for the $ca2context: =[.root, .crt, .key,
.kms.type, .kms.uri]=. also ensure that pathlen and SAN constraints are set in
the policy!

+ for =$ca2context=, you probably want pathlen set to zero or one.
+ If you plan to spin up a =k8s CA= for a cluster or for =harbor=, it's possible
  that you would need pathlen 2...
+ but any certificates issued like that should plan on SAN constraints for DNS
  for the endpoints.

This depends on how you plan to

1) distribute Root CA to trust stores
2) distribute certificate bundles to endpoints (servers and/or clients)
3) revoke trust (don't plan on it)
4) configure applications/services to rely on trust

you should generally not create a CA with pathlen >= 1 without SAN constraints
if it's network accessible. (notice how much of a PITA it was to get a signing
certificate from a classic CAB Forum vendor). here this is mitigated in three
ways:

+ expiration < 90 days (or 90 idk)
+ I need to be physically present to enter the PIN and touch
+ the Root CA is basically disposable (it's like an "anonymous CA"). generally
  not an option, for production/etc, but I can use a separate root of trust for
...
+ homelabcom ca1
  ...
  - homelabcom ca2.2...
+ homelabcom ca2
  - homelabcom ca3net @ offline?
    - jwk
  - homelabcom ca3svc @ ca.svc.home.lab.com
    - acme: device-attest-01 (yubikey:serial=12345678)
    - jwk?
  - homelabcom ca3k8s
    - no provisioners (passed as root ca to k8s)
  - homelabcom ca3harbor
    - no provisioners (passed as root ca to harbor)

workstations

+ offline
  - homelabcom ca0 -> homelabcom ca1
  - homelabcom ca1 -> homelabcom ca2
+ adhoc
  - homelabcom ca2 -> homelabcom ca3 ... ?
+ bootstrap svc
  - homelabcom ca3svc -> leaf
+ bootstrap k8s
  - homelabcom ca3cluster -> homelabcom ca3x

** home.lab.com ssh ca

This is separate from the above CA. SSH doesn't validate the trust chain
...
+ offline
  - homelabcom sshca0 -> sshca1
+ adhoc
  - homelabcom sshca1 -> ssh hosts
    - should only be needed when enrolling new devices
  - homelabcom sshca1 -> ssh principles (admin)
+ bootstrap
  - homelabcom sshca1 -> ssh principles (ansible/etc)
  - homelabcom sshca1 -> ssh principles (... me?)
  ...
  age:
    ...
    - recipient: age10g7nyzg2llvav0hjyrt3g8yet2huhe4vq9c5jmsde9x4sn7j9vzsy6s2d9
      enc: |
        ...
        -----END AGE ENCRYPTED FILE-----
  lastmodified: "2025-11-24T11:59:42Z"
  mac: ENC[AES256_GCM,data:jVNouyQOqXNSWtFeZB0HHtbeG2YHaQPMBEflFs4KdNzETQwSfwl9UamHzJNQMBwu8KjyXH9KKNKDEM10jRRPZ6HgIy1ZFHY8z7nSP/rjlKsHzuE78GjHm5CKgSXWUh+DmHLwaBx2+LNnZJFOxFuA78zyN02U24J7FYqtceJP5JQ=,iv:ql8ZyIR33CN3flVt5bShmXbjEy+ECRkg5HUdd2PRoSI=,tag:7U/PJ5d8viPaomPrxVduPA==,type:str]
...
***** using =age-plugin-tpm=

+ If =TPM2TOOLS_TCTI= isn't set, then =-T $interface= is required
  - I need to figure out how to add & remove an identity. otherwise it's stuck
    ... or worse, transient.
+ If adding =security.tpm2= support to NixOS with udev, then a reboot is likely
  required so that =tss= user/group owns =/dev/tpm(rm)?[0-9]=.
  - =tpm= devices are owned by =tss= user
  - =tpmrm= devices are accessible to =tss= group
+ Identities/handles managed under SRK need to be persistent, not transient.
  - You would need to load an ISO to create the identity, then save the
    =~/.age/identity.txt= pubkey
+ Depending on the status of [[https://github.com/Foxboron/age-plugin-tpm/issues/28][foxboron/age-plugin-tpm#28]], tpm access is required
  to encrypt secrets, even though it's asymmetric. it's possible to do this
  anyways, outside of =age= -- idk what bech32 is though. probably not a radiohead
  reference. see [[https://github.com/C2SP/C2SP/pull/96][C2SP/C2SP#96]]. to me, writing standards like this seems a little
  daunting.
+ the TPM isn't magical. here, it basically ensures that the contents of
  secrets are only available if decrypted on the machine. the main benefit is
  that someone needs to be on the machine to decrypt other secrets, so you at
  least have specified the point where they attack (which you can watch)
  - without some intermediary (and permissions/access model), anything that can
    ask the =tpm= to do anything can ask it to perform decryption.
    - getting around that requires a broker (nope), keywrapping
      (redundant/brittle)...
    - or an activation service that can decrypt, then recrypt with a wrapped
      key. i.e. you'd also have an age identity (or a adhoc key) for the
      consuming service, which requires path-activation before it starts. idk if
      it checks out..... probably dumb.

Check capabilities and SRK handles.

#+begin_src shell
sudo tpm2_getcap -T device:/dev/tpm0 handles-permanent
sudo tpm2_getcap -T device:/dev/tpm0 handles-persistent
sudo tpm2_getcap -T device:/dev/tpm0 handles-transient  # these reset on reboot
sudo tpm2_getcap -T device:/dev/tpm0 handles-nv-index   # nvram?

# supported commands
sudo tpm2_getcap -T device:/dev/tpm0 commands
#+end_src

Hmmmm.... I really just don't want a read on an =/etc= file to pull my key out

#+begin_quote
I really have no fucking clue. [[https://github.com/Foxboron/age-plugin-tpm/blob/master/plugin/plugin.go#L74-L83][obviously, age-plugin-tpm can create an object
under the SRK]], but i'm not sure how this corresponds to [[https://tpm2-tools.readthedocs.io/en/latest/man/tpm2_evictcontrol.1/][tpm2_evictcontrol]] or the
need to manage contexts (where [[https://tpm2-tools.readthedocs.io/en/latest/man/tpm2_readpublic.1/#serialize-an-existing-persistent-object-handle-to-disk-for-later-use][tpm2_readpublic mention of serializing the
handle]]). And holy shit whoever writes TPM2 with FFI/scripts must be some kind of
human/computer hybrid i mean what the fuck?

Some of these concepts circumvent what I'm overthinking below regarding
keywrapping maybe?

+ age-plugin-tpm flushes the handle, since it's a self-contained tool.
+ But it also writes [[https://github.com/Foxboron/age-plugin-tpm/blob/master/plugin/plugin.go#L139-L145][SRKName]] (the handle reference)
  - and then runs [[https://github.com/Foxboron/age-plugin-tpm/blob/master/plugin/identity.go#L116-L131][EncodeIdentity]] when it marshalls the identity
  - this passes this representation through age's =plugin.EncodeIdentity()=
  - and finally writes to =age-identity.txt=
+ The CLI logic for =HandleIdentity()= and =HandleRecipient()= handles in reverse
#+end_quote


#+begin_src shell
# testtpm1=alice.age.tpm

#+end_src

***** Using =ssh-keygen= and =ssh-to-age=
...
+ See notes on [[https://smallstep.com/docs/step-ca/policies/#policy-evaluation][Certificate Issuance Policy vs Name Constraints]]
  - policies to constraints are 1:1
  - name constraints are applied by provisioner
...
@startjson
{
  ...
  "authority": {
    ...
    "provisioners": [
      {
        "type": "ACME", "name": "acme-example", "forceCN": true,
        "claims": { "maxTLSCertDuration": "8h", "defaultTLSCertDuration": "2h" },
        "termsOfService": "",
        "website": "",
        "caaIdentities": [],
        "challenges": [ "http-01", "dns-01", "tls-alpn-01", "device-attest-01" ],
        "attestationFormats": [ "apple", "step", "tpm"],
        "attestationRoots": "",
        "options": { "x509": { "templateFile": "templates/certs/x509/default.tpl" } }
      },
