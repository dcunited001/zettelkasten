...
** Pull container and build vyos ISO
...
** Build container, then build VyOS ISO

The build scripts now run =git checkout= which will fail from within a =repo=
checkout, since =.git= isn't local to the volume that vyos uses as workdir
=/vyos=....

also, the =vyos/vyos-build:current= image lacks =syft= which is required for SBOM

+ correction: =docker pull vyos/vyos-build:rolling= is now online

*** Build docker container

#+begin_src sh :dir /data/ecto/vyos/vyos/vyos-build-lab
# from vyos-build, build an alternate vyos/vyos-build:rolling tag
tag=vyos/vyos-build:rolling
...
#+begin_src sh :dir /data/ecto/vyos/vyos/vyos-build-lab
tag=vyos/vyos-build:rolling
docker run --rm -it -v "$(pwd)":/vyos -w /vyos --privileged \
