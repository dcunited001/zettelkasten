...
+ [[https://github.com/Mic92/sops-nix?tab=readme-ov-file#templates][sops-nix templates]]

*** TPM Setup

#+begin_src shell
age-plugin-tpm --generate | gpg -aer me@fdsa.com > age-identity.txt.asc
#+end_src
...
#+begin_quote
If you change your UEFI/BIOS settings or update your kernel in a way that alters
the PCR state (like Secure Boot measurements changing), the TPM will unseal the
key successfully only if you accounted for those specific registers.
#+end_quote
...
# TODO: finalize command and make homeomorphic
sops --encrypt --output secrets.env.json path/to/your/client.ovpn
#+end_src
...
in
{
  ...
  sops = {
    defaultSopsFile = ''./hosts/${networking.hostName}/secrets.yaml'';
    # validateSopsFiles = false;
  }

  # "Make sure permissions on this file are 0600!" -- LLM
  # 
  # sops.age.keyFile = "/var/lib/sops/age-identity.txt";

  sops.secrets = {
    "services/postgres_password" = {
      owner = "postgres";
      group = "postgres";
      mode = "0400";
    };'
...
keys:
  # ...
  - &hosts:
    - &tpmhost age1jzrpvffcwjtv4e77v24ye44z2yk3nk3hkyjk60tfluuaxszdtf7s0u3xeh

      - path_regex: ./hosts/tpmhost/secrets.yaml$
          key_groups:
          - age:
            - *tpmhost
#+end_src
