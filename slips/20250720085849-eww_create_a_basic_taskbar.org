...
+ [[https://github.com/sagayda/dotfiles/tree/5af0f8727f49a318bae7df2249c801ae8c1fa32a/eww/.config/eww][sagayda/dotfiles eww/.config/eww]].
  - see [[file:img/eww/sagayda-eww-bar.diff][img/eww/sagayda-eww-bar.diff]] which stubs out most of the CLI calls
  - this contains some wireplumber lua scripts which monitor volume for
    i think while responding to changes in the audio graph
  - clone the repo, apply the diff, then startup =eww= with the correct
    configuration dir or export the variable in the environment
  - run =eww open bar --screen n= 
  - run =eww open system-menu --screen n= for the pipewire controls
    - these source =suka.json= for initial configuration (or something)

**** wpexec

Running these requires the lib =json.lua= in the same dir.

+ I'm not sure these scripts are actually wired up.

***** sinks1.lua

#+begin_src plantuml :noweb yes :file img/eww/ewwWpSinks1.svg
' sh :noweb yes
@startjson
<<jsonEwwWpSinks1>>
@endjson
...
#+RESULTS:
[[file:img/eww/ewwWpSinks1.svg]]

#+name: jsonEwwWpSinks1
#+begin_src json :noweb-ref jsonEwwWpSinks1
{
  "sources": [{"volume":0.74508667725208,"mute":false,"id":123,"description":"muh webcam","name":"alsa_input.usb-muh-webcam.analog-stereo"}],
  "default_source": {"volume":0.74508667725208,"mute":false,"id":123,"description":"muh webcam","name":"alsa_input.usb-muh-webcam.analog-stereo"},
  "sources_count": 1,
  "sinks": [{"volume":0.77999997037298,"id":34,"description":"Main Output","name":"Main-Output-Proxy"},
            {"volume":0.8670349240706,"id":77,"description":"muh sound card","name":"alsa_output.pci-0000_da_d0_1.analog-stereo"}],
  "default_sink": {"volume":0.77999997037298,"id":34,"description":"Main Output","name":"Main-Output-Proxy"},
  "sinks_count": 2
}
#+end_src
