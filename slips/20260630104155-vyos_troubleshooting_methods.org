...
+ =monitor command diff= i think runs =watch -d "conntrack -L | sort"= under the
  hood. There are also newer options like [[https://github.com/blacknon/hwatch][hwatch]]

** Routing

+ monitor command diff 'show ip route 1.2.3.4' :: shows when routes change for
  default gateway. OSPF was not making failover node aware of alternate default
  gateway path.

** Firewall

+ monitor traffic interface any 'host 8.8.8.8' :: ping 8.8.8.8
+ monitor command diff 'show firewall ipv4 forward filter' :: check forward/rpf
  - run on both

*** Conntrack

+ =show conntrack-sync cache [internal | external]= shows refined output for
  =conntrack -L=
+ =monitor command diff 'show conntrack-sync cache internal | sort | uniq'=
  starting this at the same time on both routers will demonstrate how
  =disable-external-cache= causes the same "shapes" to appear for NAT sessions in
  =internal= cache
