...
#+begin_quote
Note! Vyos config syntax is picky. An ISO booted with a config that can't be
parsed will probably lock you out. This is easier with PXE/TFTP... but still
slow. The command syntax is easier and =show configuration= will dump a mostly
useful configuration.

+ The comment syntax below is incorrect
+ some declarations like =offload { gro }= require one option per line and can
  fold the braces onto one line
#+end_quote
...
function. A tunnel/vpn may help.
...
#+begin_src shell :var domain=eg.tld name=home type=cname args='()
d=$domain
n=$name
a=${args[@]}
keywords=
