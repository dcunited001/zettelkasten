...
Available in almost every subcommand where it makes sense:

| ~--context=$name~ | ~--ca-url=$uri~ | ~--root=$pem~ |

Admin credentials and options are mostly used for [[https://smallstep.com/docs/step-ca/provisioners/#remote-provisioner-management][remote provisioner management]],
but are available on most commands.

| ~--admin-cert=$chain~       | ~--admin-key=$keyfile~  | ~--admin-password-file=$file~ |
| ~--admin-subject=$subject~  | ~--admin-name=$subject~ |                             |
| ~--admin-provisioner=$name~ | ~--admin-issuer=$name~  |                             |

For clarity, those are excluded from the counts below

*** KMS

Commands that allow specifying =kms= options:

+ step ca init :: KMS type and config
  - --kms=$type ::
  - --kms-root=$uri ::
  - --kms-intermediate=$uri ::
  - --kms-ssh-host=$uri ::
  - --kms-ssh-user=$uri ::
+ step ca certificate :: KMS and Attestation KMS
  - --kms=$uri :: Cloud KMS or HSM
  - --attestation=$uri :: KMS uri used for attestation
+ step ca {rekey,renew,sign,token} --kms=$uri :: Cloud KMS or HSM
+ step ca provisioner {add,update} :: SCEP provisioner
+ step certificate {create,sign} :: KMS and CA KMS
  - --kms=$uri :: Cloud KMS or HSM
  - --ca-kms=$uri :: signing KMS
...
#+begin_src shell :results output code :wrap example :var group="../step-cli-usage-options.mdx" n=25
grep -E '^\*\*-' "$group" | tr -d '*' \
    | grep -vE '^--admin-(provisioner|subject|key|cert|password-file)=' \
    | grep -vE '^--(context|ca-url|root)=' \
    | sort | uniq -c | sort -rnk1 | head -n"$n"
#+end_src
...
#+begin_example
     ...
     10 --kid=`kid`
      ...
      8 --kms=`uri`
      7 --size=`size`
      7 --not-before=`time|duration`
      7 --not-after=`time|duration`
      7 --format=`format`
      7 --crv=`curve`, --curve=`curve`
      7 --console
      6 --x5c-cert=`chain`
#+end_example

#+name: stepDumpOptions
#+begin_src shell :results output code :wrap example :var group="../step-cli-usage-options.mdx"
grep -E '^\*\*-' "$group" | tr -d '*' \
    | grep -vE '^--admin-(provisioner|subject|key|cert|password-file)=' \
    | grep -vE '^--(context|ca-url|root)=' \
    | sort | uniq
...
#+begin_example
     ...
     12 --eab-key-id=`value`
      ...
      2 --ssh-template=`file`
      2 --ssh-template-data=`file`
      2 --ssh-host-min-dur=`duration`
      2 --ssh-host-max-dur=`duration`
      2 --ssh-host-default-dur=`duration`
      2 --ssh
      2 --scep-decrypter-key-uri=`uri`
      2 --scep-decrypter-key-password-file=`file`
      2 --scep-decrypter-key-file=`file`
#+end_example
