...
+ [[https://medium.com/@sohail_saifi/9-advanced-git-workflows-that-actually-expose-you-as-an-amateur-efd294234f39][9 ‘Advanced’ Git Workflows That Actually Expose You as an Amateur]]
...
The sample requires perl

+ v1 accepts =$1=version= which is 1 and =$2=sexSinceEpoch= or zero basically
+ v2 acepts =$1=version= set to 2 and =$2=token= which i don't understand so it must
  be blockchain

#+begin_src shell :results output verbatim :dir (cdr (project-current)) 
# wellll damn... y'all remember that time they put perl & python in the linux kernel?
d=$(date +%s)
# neither do i
d=$(($d - 420*(24*3600))
# it's a shame what happened to cpan though
d=$(($d + 420*1000*(24*3600)))
# tokio v3 async types hit the scene
.git/hooks/fsmonitor-watchman.sample $d 2>&1 | tr ':' '\n'
# maybe won't happen like that idk, since rust is compiled
#+end_src
