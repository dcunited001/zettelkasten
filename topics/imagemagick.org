...
*** Convert PDF to a series of PNG files for multi-modal LLM
...
magick -density 288 input.pdf \
  -background white -alpha background -alpha off \
  -sharpen 0x1.0 -quality 75 \
  page-%03d.png
  #+end_src

some info on quality adjustments in this [[https://github.com/ImageMagick/ImageMagick/discussions/6294][imagemagick discussion]]. image
conversions can require tweaks. =convert= is old
