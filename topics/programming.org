...
+ [[https://arxiv.org/abs/2509.15834v1][Automatic Layout of Railroad Diagrams]]: tons of github links.
  - wrote a scala library that accepts s-expressions. [[with ][online here]]
...
**** [[https://github.com/tabatkins/railroad-diagrams][tabatkins/railroad-diagrams]]

[[https://tabatkins.github.io/railroad-diagrams/generator.html][App here]]. Syntax is a bit verbose. Intended to be used via python/typescript
...
Diagram(
  Group(
    Choice(1,
      Sequence(
        NonTerminal('Translation2d'),
        NonTerminal('Rotation2d')),
      Skip(),
      Sequence(
        NonTerminal('double'),
        NonTerminal('double'),
    ...
    'Pose2d Constructor'),
  Choice(1,
    Sequence(NonTerminal('.transformBy'), NonTerminal('Transform2d')),
    Sequence(Choice(1,
      NonTerminal('.rotateBy'),
      NonTerminal('.relativeTo')), NonTerminal('Pose2d')),
    Sequence(
      NonTerminal('.rotateAround'), 
        NonTerminal('Translation2d'),
        NonTerminal('Rotation2d')),

  Group(
    Choice(1,
      Sequence(NonTerminal('.exp'),NonTerminal('Twist2d')), 
      Sequence(NonTerminal('.log'),NonTerminal('Pose2d'))), 
    'exponential')))
#+end_src
...
**** Testing =socat= addresses

#+begin_src shell :session *guix-antlr*
guix shell python java-antlr4-runtime-python antlr4
#+end_src

#+RESULTS:
| guix                     | shell:                                                            | [1m[0mpackage  | '[1mjava-antlr4-runtime-python[0m' | has         | been   | superseded | by | '[1mpython-antlr4-runtime[0m' |
| [1;35mThe              | following                                                         | derivation         | will                                   | be          | built: |            |    |                                   |
| [0m                    | [2m/gnu/store/nixnglf0zyr69zmn5r3na46jl5mbki7v-[0mprofile.drv |                    |                                        |             |        |            |    |                                   |
|                          |                                                                   |                    |                                        |             |        |            |    |                                   |
| [1m[0m[Kbuilding | CA                                                                | certificate        | bundle...                              |             |        |            |    |                                   |
| [K[Klisting      | Emacs                                                             | sub-directories... |                                        |             |        |            |    |                                   |
| [K[Kbuilding     | fonts                                                             | directory...       |                                        |             |        |            |    |                                   |
| [K[Kbuilding     | directory                                                         | of                 | Info                                   | manuals...  |        |            |    |                                   |
| [K[Kbuilding     | profile                                                           | with               | 3                                      | packages... |        |            |    |                                   |

Build in =*guix-antlr*= shell:

#+begin_src shell
antlr4 -Dlanguage=Python3 SocatAddressLexer.g4
#+end_src

Unsure of how to get a "grammar" out of it. Would be interesting to permute as a
data-structure though. For graph traversal or smth idk. Permute topology to
expand using the opposite of edge cuts with constraints on addressing/etc for
orchestration.

#+name: antlrSocatAddress
#+begin_example antlr
grammar SocatAddress;

/////// option is a reserved word, i think
// address : type ':' options? addrBody ;
// options : (option ',')* ;

// grammar rules

fragment AddrPath : AddrType Path;
fragment AddrIP : IPAddress ':' Port;

Address : AddrType ':' Opts? AddrBody ;
Opts : Opt (',' Opts)?;
Opt  : Identifier ('=' (Identifier | Number | Path))? ;

AddrBody: Path | (IPAddress ':' Port) | AddressPair ;
...
From gemini. Not bad. A little incomplete.
...
options : (option ',')* ;
option  : Identifier ('=' (Identifier | Number | Path))? ;

addrBody: Path | IPAddress ':' Port | AddressPair ;

type    : 'TCP' | 'TCP4' | 'TCP6' | 'UDP' | 'UDP4' | 'UDP6' | 'UNIX' | 'OPEN' | 'EXEC' | 'SYSTEM' | 'STDIO' | 'PIPE' ;

IPAddress : [0-9.]+ ;
Port      : [0-9]+ ;
AddressPair : [a-zA-Z0-9.\-_]+ ':' [a-zA-Z0-9.\-_]+ ;
Path      : '/' [a-zA-Z0-9.\-/_]+ ;
Identifier : [a-zA-Z0-9\-]+ ;
Number    : [0-9]+ ;
