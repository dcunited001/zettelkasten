...
Can also print to PDF with:

#+begin_src shell
# this requires setup of a default PDF printer in cups, which idk that I want
lpr  -o outputfile=gnu-screen-terminal-redirection.pdf src/doc/fdpat.ps

# SCREENSRC=/tmp/tmp.fdsa/screen
nix shell nixpkgs#imagemagick nixpkgs#ghostscript
magick  -density 288 ./src/doc/fdpat.ps \
    cheatsheets/gnu-screen-terminal-redirection.pdf
#+end_src

This required adding octal =\251= to place a copyright. This should definitely be
in the GNU screen docs: both as a PDF itself and also made more clear in the manual

#+begin_example diff
diff --git a/src/doc/fdpat.ps b/src/doc/fdpat.ps
index 6b10099..e42c3be 100644
--- a/src/doc/fdpat.ps
+++ b/src/doc/fdpat.ps
@@ -6488,7 +6488,7 @@ Courier 8 SetF
 [ 1 0 0 1 22 797 ] concat
 %I
 [
-(Subshell)
+(Subshell \2512007 FSF)
 ] Text
 End
#+end_example
