...
+ ANF can have no negation: Arbitrary predicates need to be symbolically
  rewritten into ANF before these methods are practical
+ ANF is a [[https://en.wikipedia.org/wiki/Canonical_form_(Boolean_algebra)][Canonical form]] for boolean algebra. Many predicates/statements will
  reduce to ANF if they always result in the same value for all parameters.

***** [[https://www.leviathansecurity.com/blog/encoding-bitwise-functions-as-polynomials][Encoding bitwise functions as polynomials]]

Article about predicate logic. implements ALU in python.

+ i've done ALU: [[id:671bb886-393a-47e8-9dd1-835f9e846fac][Python: Adder Using Recursive Xor]] at least for an adder.
+ the author's points about some of the profundities are interesting.
  - it seems like maybe he solved the problems himself. i wonder if anyone put
...
See [[https://en.wikipedia.org/wiki/Algebraic_normal_form#The_Pascal_method][Pascal Method & Summation Method]] for generation of underlying structure for
dynamic programming.

(i'm blazing through this too quickly) the summation method seems to indicate
containers for bits in the algebraic structure.

how to supply coefficients?

+ precompute the sierpenski's gasket shape, retrieve the necessary table for
  size =2^n=. using a data structure that supports large bitmasking ops.
  - this isn't spatially efficient (that's very difficult...)
+ a*b*c == a^b^c :: all the rows whose have some variable as zero become zero.
  - it looks like you need to iterate, since the tables shift to tables for =n-1=
    as args are eliminated. either evaluate them as a list...
  - or instead, retrieve a precomputed bitmask
    - OpenGL API constructs like [[https://wikis.khronos.org/opengl/Blending][blending]] and [[https://wikis.khronos.org/opengl/Logical_Operation][logic]], combined with textures &
      various techniques allow you to be creative in how values accumulate at
      the end of computation (in GPU, i think supported by =PTX= since "clear
      screen" has similar requirements to "go fast, set same value everywhere")
    - CUDA or FPGA would be more efficient.
+ the mask is easy to precompute (or generate in shader/cuda with indices)
  + then you just =xor= the rowmask with the sierpenski's, resulting in bits
+ run map-reduce over the texture, accumulating popcount.
  - if the number is odd, the predicate true; even, it's false
  - i think (could be backwards.)

This runs much faster in FPGA/ASIC, but i think you're spatially bound there
(esp. with interference delay/feedback from cumulative FPGA/ASIC computation
stages) ... or whatever.

#+begin_quote
I've been in roanoke for a very long time, but i do remember learning about
cumulative delay/feedback in boulder. people conversed about technology.
#+end_quote

**** Cellular Automata

+ [[https://arxiv.org/pdf/1212.3069][4-state Sol'n to Firing Squad Synchronization Problem (via rules 60,102)]]
  - lol, the what? [[https://en.wikipedia.org/wiki/Firing_squad_synchronization_problem][Firing Squad Synchronization Problem]]
  - [[https://mathworld.wolfram.com/Rule60.html][Rule 60]] is approximated via Pascal's Triangle (generated from seq?) and
    looks like sierpenski's
