...
There are other prefixes: =$n.xz=, =$n=, =${n}ssl=, ...

#+begin_src shell
q=guix
man --where --regex --names-only --global-apropos -- "$q" \
    | sed -e 's/^.*\///g' \
    | rev | cut -f2- -d'.' | rev \
    | sed -E 's/(.*)\.([0-9])(ssl\.gz|\.gz|\.xz)$/\1 \2/g'
#+end_src

***** Build a graph from "See Also"

Run =env MANWIDTH=100000000 man --pager=cat $page= and =man= will drop =COLUMN= to
only display a single word.

#+name: awkManSeeAlso
#+begin_example awk
BEGIN { atSee=0; atSeeAlso=0; }
($0 ~ /SEE$/) { atSee=1; next; }
($0 ~ /ALSO$/) { atSeeAlso=1; next; }
(atSee) && (atSeeAlso) && ($0 ~ /^$/) { exit; }
(atSee) && (atSeeAlso) { print $1; next; }
#+end_example

#+name: manSeeAlso
#+headers: :var awkProg=awkManSeeAlso page="ossl-guide-libraries-introduction"
#+begin_src shell :results output code :wrap example
# yayyy... ${page%%.7ssl.gz}

env MANWIDTH=100000000 man --pager=cat "$page" | awk "$awkProg"
#+end_src
