...
#+begin_src shell :results output verbatim :var css=manhtmlcss
# doesn't include manhtml() in scope ... or a mandb apparently
# ... org-babel also cleans tmp files it creates

d="$(mktemp -d)"
# d=/tmp/tmp.yK7uS7yF2N
regen=1

# for mp in bluetoothctl{,-advertise,-endpoint,-hci,-monitor,-scan,-admin,-assistant,-gatt,-mgmt,-player,-transport}; do
for mp in containers{.conf,-storage.conf,-registries.conf,-policy.json}; do
  [[ 1 -eq "$regen" ]] && manhtml $mp > $d/$mp.html
  # these should be dt (mostly)
  sed -i -E 's/<p style="margin-left:9%; "><b>/<p class="mandt"><b>/g' $d/$mp.html
  # these should be dl (but may lack dt)
  sed -i -E 's/<p style="margin-left:9%; ">/<p class="mandl">/g' $d/$mp.html
  # inject the styles ... ¯\_(ツ)_/¯ lol
  sed -i -E 's/ +<\/body>//g' $d/$mp.html
  sed -i -E 's/ +<\/html>//g' $d/$mp.html
