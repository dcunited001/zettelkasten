...
- HyperPolyglot:[[https://hyperpolyglot.org/unix-shells#startup-file][Unix Shells: Bash, Fish, Ksh, Tsch, Zsh]]
...
**** Adjusting =paste= output while avoiding weird =tput=
***** Original Attempt
:PROPERTIES:
...
+ this outputs a babel block that outputs a verse block...
+ but needs some manual munging to avoid dynamic re-evaluation of commands to
  discover widths. it's "good enough"
...
***** Dynamically
:PROPERTIES:
:header-args+: :dir (getenv "OMARCHY_PATH")
:END:

And this monstrosity is what we call ~a solution looking for a problem~

I subtly changed what i was querying and suddenly, it wasn't so simple

#+begin_src shell :results output code :wrap verse
# qmlfiles=$(fd -H '.*\.qml' .)
# from $OMARCHY_PATH
# manifestdir=$(dirname $m) # treeinput+=($m)
# treeinput+=($(jq -r --arg p "$manifestdir" '[$path, (.entryPoints | to_entries[] | .value)] | join("/")' $m));

# manifests=$(fd -L manifest.json "shell" | sort | uniq)
get_plugin_entry_points() {
    local path=$1
    
    manifests=$(fd -L manifest.json "$path" | sort | uniq)
    treein=()
    for m in ${manifests[@]}; do
        mroot=$(dirname $m)
        jqout=$(jq -r --arg d "$mroot" '[$d, (.entryPoints | to_entries[] | .value)] | join("/")' "$m")
        treein+=($(echo $jqout))
    done
    echo "${treein[@]}" | tr ' ' '\n'
}
...
treedims=()
plugin_roots=($(find -H "shell/plugins" -maxdepth 1 -type d | grep -v "shell/plugins\$"))
for p in ${plugin_roots[@]}; do
    treedims+=($(get_plugin_entry_points $p | tee -p >(wc -l) >(wc -L) | tail -n2))
done
echo ${treedims[@]}

# terminate early bc i don't feel like it
treeinputs=()
for ((i=0; i<${#plugin_roots}-4; i+=4)); do
#    paste <(get_plugin_entry_points $p | tree -l --fromfile . | tput)
    p1=($(get_plugin_entry_points ${plugin_roots[$i]}))
    p2=${plugin_roots[$((i+1))]}
    p2=($(get_plugin_entry_points $p2))
    p3=${plugin_roots[$((i+2))]}
    p3=($(get_plugin_entry_points $p3))
    p4=${plugin_roots[$((i+3))]}
    p4=($(get_plugin_entry_points $p4))
paste <(echo ${p1[@]} | tr ' ' '\n' | tree --fromfile . --noreport) \
    <(echo ${p2[@]} | tr ' ' '\n' | tree --fromfile . --noreport) \
    <(echo ${p3[@]} | tr ' ' '\n' | tree --fromfile . --noreport) \
    <(echo ${p4[@]} | tr ' ' '\n' | tree --fromfile . --noreport) \
   | awk -F'\t' -v w1="${treedims[$((2*i))]}" \
    -v w2="${treedims[$((2*i+2))]}" \
    -v w3="${treedims[$((2*i+4))]}" \
    -v w4="${treedims[$((2*i+6))]}" \ '
    { printf "%-*s  %-*s %-*s  %-s\n", w1, $1, w2, $2, w3, $3, $4 }
...
#+RESULTS:
#+begin_example
30 1 39 1 44 9 37 1 42 1 31 1 42 1 30 1 41 1 39 1 25 1 45 12 36 1 40 1 54 4
.                               .                                       .                                             .
└── shell                       └── shell                               └── shell                                     └── shell
    └── plugins                     └── plugins                             └── plugins                                   └── plugins
        └── agents                      └── background                          └── bar                                       └── clipboard
            └── Panel.qml                   └── Background.qml                      ├── Bar.qml                                   └── Clipboard.qml
                                                                                    └── widgets                       
                                                                                        ├── ActiveWindow.qml          
                                                                                        ├── Indicators.qml            
                                                                                        ├── KeyboardLayout.qml        
...
└── shell                                   └── shell                       └── shell                                   └── shell
    └── plugins                                 └── plugins                     └── plugins                                 └── plugins
        └── dev-gallery                             └── emojis                      └── image-picker                            └── lock
            └── GalleryPanel.qml                        └── Emojis.qml                  └── ImagePicker.qml                         └── Service.qml
.                                          .                                       .                          .
└── shell                                  └── shell                               └── shell                  └── shell
    └── plugins                                └── plugins                             └── plugins                └── plugins
        └── menu                                   └── notifications                       └── osd                    └── panels
            └── Menu.qml                               └── Service.qml                         └── Osd.qml                ├── audio
...
└── shell                             └── shell                                └── shell                                               
    └── plugins                           └── plugins                              └── plugins                                         
        └── polkit                            └── reminders                            └── services                                    
            └── PolkitAgent.qml                   └── ReminderFlow.qml                     ├── battery                                 
                                                                                           │   └── Service.qml                         
                                                                                           ├── idle                                    
                                                                                           │   └── Service.qml                         
                                                                                           ├── media                                   
...
***** Looping over =DETERMINISTIC= outputs to discover

Probably the simplest with re-evaluation

#+begin_src shell
# declare resWidth=()
# declare resLength=()
# resWidth+=($(tree "$pp" --noreport | wc -L))
declare resDims=() # width, height
for pp in ${paths[@]}; do
    resDims+=(tree -P 'manifest.json' "$pp" | tee -p >(wc -l) >(wc -L) | tail -n2)
done

W1=$()
W2=$(tree ./muh/result2 --noreport | wc -L)
W3=$(tree ./muh/result3 --noreport | wc -L)

# 2. Paste them side-by-side using the precise calculated widths (plus 2 spaces for padding)
paste \
  <(tree ./baulig/packages --noreport) \
  <(tree ./gocix/modules --noreport) \
  <(tree ./guix-crypto/src --noreport) \
| awk -F'\t' -v w1="$W1" -v w2="$W2" -v w3="$W3" '
    { printf "%-*s  %-*s  %-s\n", w1, $1, w2, $2, $3 }
  '
#+end_src
...

=ed7t
-----END PGP MESSAGE-----
...
[[https://www.youtube.com/watch?v=2A4bs40scSo][Extremely Cool Shell Trick]] has good explanations on what procsub is doing.
Apparently =ls -l >(cat)= will show a link. It illustrates why this works (bc it
creates a file descriptor in the =/proc= tree)

#+begin_src shell
tar cf - directory \
    | tee >(ssh thisserver tar xf -) \
    | tee >(ssh andthatserver tar xf -) > /dev/null
#+end_src

Also another useful trick to =tail= and store store logs

#+begin_src shell
tmp=$(mktemp); # this is now less necessary
debug_some_service > >(less) 2>(tee $tmp)
#+end_src

And possible to procsub into editor:

#+begin_src shell
# run `:w !cat > /dev/stdout` # ... not the same terminals for processes
vim -f --not-a-term <(echo $(date +%s | sha256sum)) | cat # >(cat)
# nothing quite worked though. the -f option sounded close, but vim's not
# expecting to write to /dev/stdout ... still useful though
#+end_src
