...
*** Icons
**** GTK Icon Themes
These need to be set in =~/.config/gtk*=. Can check via =dconf= or its GUI.

+ For me, on NixOS (no HM), i can set in =dconf= which isn't automatically
  reflected in =~/.config/gtk-3.0= (or elsewhere)
+ /org/gnome/desktop/interface/icon-theme :: in Adwaita (or whatever)
...
| Single | Multiple | Desc                                                                                               |
|--------+----------+----------------------------------------------------------------------------------------------------|
| %f     | %F       | Single file (including the path), even if multiple files are selected.                             |
| %u     | %U       | URL/file                                                                                           |
| %i     |          | The Icon key, exp. into 2 args: --icon and then the value of the Icon key                          |
| %c     |          | The translated name of the application as listed in the appropriate Name key in the desktop entry. |
| %k     |          | Desktop file location as URI (e.g. vfolder)                                                        |
