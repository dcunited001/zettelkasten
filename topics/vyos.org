...
Build the docs, convert to docset using [[https://github.com/ppwwyyxx/dash-docset-tensorflow/blob/master/generate-docset.sh][dashing]]. See [[id:765d942f-30be-49ae-abc3-f16ddd5440bc][VyOS: Build Dash Docsets]]

#+begin_src shell
# git clone git@github.com:vyos/vyos-documentation
podman build -t vyos/vyos-documentation docker
podman run --rm -it \
    -v "$(pwd)":/vyos -w /vyos/docs \
...
+ [[https://docs.vyos.io/en/latest/installation/install.html#pxe-boot][Build PXE Image]]
  - [[https://forum.vyos.io/t/cannot-install-vyos-after-booting-into-live-with-pxe/13704][Cannot install VyOS After Booting Into Live with PXE]] covers setup to PXE
    boot an installable image.

*** Validate config without running ISO

Apparently you can test the config on the ISO using =make testc=. It runs
=./scripts/check-qemu-install=: read for info on the arguments it expects. if it
works, this is incredibly useful. I want the ISO to boot up with SSH ... and idk
cloud config or other options soo.... When it doesn't boot, I'm back to
sneakernet (or jumping through VTYs until I can hack the ISO to boot in the
fallback config in order to find the config line.)

+ [[https://www.reddit.com/r/vyos/comments/1j9fyh4/best_way_for_config_validation_in_cicd/][Best way for config validation in CI/CD?]]
...
Container-based networks with GUI toolkit and VSCode integration

*** GNS3

QEMU-based networks with GUI toolkit

+ [[https://docs.vyos.io/en/rolling/installation/virtual/gns3.html][VyOS docs for GNS3]]
+ [[https://haryachyy.wordpress.com/2026/06/18/build-a-network-lab-on-your-laptop-gns3-with-vyos-and-vpp-step-by-step/][Build A Network Lab On Your Laptop: GNS3 With VyOS and VPP]]
  - Covers configuring VPP (Vector Packet Processing), a high-perf userspace
    dataplane. VyOS (1.5+) wires it up through LCP (Linux Control Plane)
  - Also covers virtualized DPDK
...
[[https://forum.vyos.io/t/how-does-persistence-work-for-vyos-on-bare-metal-installs-between-upgrades/14862][How does persistence work for VyOS on bare metal?]]

| ext4     | /dev/sda3  | ~/usr/lib/live/mount/persistence~                            |
| squashfs | /dev/loop0 | ~/usr/lib/live/mount/rootfs/999.202604xxxxxx.squashfs~       |
| overlay  |            | ~/~                                                          |
| overlay  |            | ~/config~                                                    |
| ext4     | /dev/sda3  | ~/boot~                                                      |
| ext4     | /dev/sda3  | ~/boot/grub~                                                 |
| ext4     | /dev/sda3  | ~/usr/lib/live/mount/persistence/boot/999.202604240749/grub~ |

+ /usr/lib/live/mount/persistence :: persists data between reboots
  - ./boot{,/grub} :: 
  - ./container :: not copied during upgrades
+ /config :: overlaid from the squashfs built at each upgrade (& install?)

Other directories in =./persistence= are not managed by VyOS during upgrade and
are not overlaid onto (sourced from the squashfs loaded during initramfs)

The mounts in =/etc/fstab= are order-sensitive ("duh?"... i guess, it's an
overlay? idk). There are race conditions mentioned for some services &
interfaces, so it would be safer to stick to one main file system until you're
sure.

It's super handy to be able to run containers on the router, esp. during the
homelab bootstrap. Apparently, it's got a =TFTP= server, which i somehow missed...
and it can run in a VRF which makes it a bit easier to control.
...
#+begin_quote
Note: this actually doesn't work so well
#+end_quote
