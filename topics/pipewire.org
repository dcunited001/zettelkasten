...
** Tools
+ [[https://github.com/pipeweaver/pipeweaver][pipeweaver/pipeweaver]]
...
*** Config
+ use =pw-config paths= to check paths that load configuration.

+ via [[https://discourse.nixos.org/t/pipewires-extraconfig-wireplumbers-extraconfig-virtual-microphone-source-confusion/68154/6][NixOS Discourse =pipewire.extraConfig= and =pipewiere.wireplumber.extraconfig=]]
  - [[https://sorrel.sh/blog/pipewire-how-to-static-loopbacks/][How To: Uncomplicated Loopbacks]]
  - [[https://github.com/IceDBorn/pipewire-screenaudio][IceDBorn/pipewire-screenaudio]] passthrough pipewire audio to WebRTC
  - [[https://www.reddit.com/r/pipewire/comments/1awv14w/when_creating_a_new_virtual_sink_how_do_you/][How to specificy a format besides F32P when creating a virtual sink?]]

+ [[https://discourse.ardour.org/t/wireplumber-best-practices/111163/2][Convo on Ardour Forums about =media.roles= in pipewire/wireplumber]]
  - most applications don't really set =media.roles=. personally, idk if this is
    set on source/stream node init by applications which do -- or more
    importantly, whether Lua hooks would continue to fire if it's changed after
    the fact.
...
Haven't found any event-based =media.role= hooking

+ modular configuration for specific devices in [[https://github.com/notashelf/nyx/blob/d407b4d6e5ab7f60350af61a3d73a62a5e9ac660/modules/core/common/system/hardware/multimedia/sound/servers/pipewire/wireplumber/devices.nix#L12][notashelf/nyx]]
+ matching on =media.role= to change stream properties in [[https://github.com/JManch/nixos/blob/ee33781b763207d1c82a688b8a8e9d009445e359/modules/nixos/system/audio.nix#L95][JManch/nixos]]


*** Config

+ [[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/src/config/wireplumber.conf#L1][src/config/wireplumber.conf]] the base configuration
  - in +lua+ [[https://pipewire.pages.freedesktop.org/wireplumber/daemon/configuration/conf_file.html#the-spa-json-format][SPA JSON]] syntax... which is also used for pipewire
+ See [[https://pipewire.pages.freedesktop.org/wireplumber/daemon/locations.html#configuration-fragments][Configuration Fragments]] for load order. on NixOS without much other
  configuration, this includes what's built into

I guess i didn't get the [[https://www.collabora.com/news-and-blog/blog/2022/10/27/from-lua-to-json-refactoring-wireplumber-configuration-system/][push notification]]. I think the Lua system is still used
for scripting, just not top-level configuration.

| SPA JSON       | Old Lua Config                                       |
|----------------+------------------------------------------------------|
| policy.conf    | 10-default-policy.lua                                |
| access.conf    | 20-default-access.lua                                |
| alsa.conf      | 30-alsa-monitor.lua, 50-alsa-config.lua              |
| libcamera.conf | 30-libcamera-monitor.lua, 50-libcamera-config.lua    |
| v4l2.conf      | 30-v4l2-monitor.lua, 50-v4l2-config.lua              |
| device.conf    | 40-device-defaults.lua, 50-default-access-config.lua |
| stream.conf    | 40-stream-defaults.lua                               |

**** Scripts

Similar setup to systemd (provides, requires)
*** Scripting

+ [[https://blog.wido.dev/wireplumber-scripting][Wireplumber Scripting]] practical example from end-to-end
+ [[https://gist.github.com/THS-on/a8fe22891c1008454a948e86d25315f5][auto_attach_filter.lua]]
+ [[https://github.com/bennetthardwick/dotfiles/tree/master/.config/wireplumber][bennetthardwick/dotfiles]]
+ lua scripts can be loaded into [[https://github.com/bennetthardwick/dotfiles/blob/master/.config/wireplumber/wireplumber.conf.d/99-auto-connect-ports.conf][wireplumber.components]]

**** Logging

[[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/docs/rst/daemon/logging.rst#L10][Categories]] and Levels:
...
+ It supports globs (which is nice to know)
+ =wpctl set-log-level D= then =journalctl --user -xb -u wireplumber -f=
+ or =wpctl set-log-level 'W,wp-*:D'=

| wp-*            | libwireplumber              |
| wplua           | LOG Topic For C++ Side      |
| s-linking       | ~linking/*.lua~ scripts       |
| s-default-nodes | ~default-nodes/*.lua~ scripts |

You're going to want at least ~s-*:D~ for lua scripts. (org won't let me add
...
=wpexec= can run arbitrary scripts and it loads the proper environment (inside
=Core=, i think)
...
[[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/src/tools/wpexec.c#L33-62][=wpexec= only accepts two args]]: =wpexec $script $spaJson=

**** Modules
These are all in [[https://gitlab.freedesktop.org/pipewire/wireplumber/-/tree/master/src/scripts/lib?ref_type=heads][./src/scripts/lib]]

#+begin_src shell :results output verbatim :dir /data/ecto/pipewire/pw/wireplumber
grep -re 'require \?(' pw/wireplumber/src/scripts
    | sed -E 's/.*:(.*) = require ?\("(.*)"\)/\/\1/g'
    | sort
    | tree --fromfile .
#+end_src

| agutils | audio-group-utils |
| cutils  | common-utils      |
| devinfo | device-info-cache |
| futils  | filter-utils      |
| lutils  | linking-utils     |
| mutils  | monitor-utils     |
| nutils  | node-utils        |

#+begin_src dot :file img/wireplumber-lua-utils.svg
digraph G {
    agutils;
    {futils,lutils,nutils} -> cutils;
    devinfo;
    mutils;
}
#+end_src
...
#+RESULTS:
[[file:img/wireplumber-lua-utils.svg]]

Modules imported by each script

#+begin_src shell :results output verbatim :dir /data/ecto/pipewire/pw/wireplumber
find src/scripts/ -name '*.lua' \
    -exec grep -e 'require \?(' \{\} + \
    | sed -E 's/:.*require ?\("(.*)"\)/\/\1/g' \
    | tree --fromfile .
#+end_src
...
There is a concept of global state:

+ some variables must be global
+ other locals cant default to splat =...=

**** Constraints

[[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/modules/module-lua-scripting/api/api.lua#L10][Constraint (spec)]] either the symbol or the verb is appropriate. each one has
valid arity.
...
See [[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/src/scripts/device/automute-alsa-routes.lua#L195-212][automute-alsa-routes.lua]] which responds to events by un/hooking event
handlers

**** Notifications

There's a plugin for =notify-send= via d-bus which loads in the default
configuration

+ IDK whether =Plugin.find("notifications-api")= needs to run in the correct scope
+ It doesn't do anything, regardless of where this block runs
  - inside an =ObjectManager= block, in a Core block afterwards, by itself
  - =Core.require_api('api.notifications', function(...) ... end) block=
...
I guess the docs address this, so does the code.

+ [[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/modules/module-lua-scripting/api/api.c#L305-318][modules/module-lua-scripting/api/api.c#L305-318]]
  - warns if runs in daemon context
+ [[https://gitlab.freedesktop.org/pipewire/wireplumber/-/blob/0dad52f7741726f0cd7917f9ad521a77bd50eb31/modules/module-lua-scripting/api/api.c#L273-285][modules/module-lua-scripting/api/api.c#L273-285]]
  - this is the definition of the Lua table associated to Core

#+begin_src shell
systemctl restart --user pipewire
wpctl set-log-level 'W,m-*:I,wp-*:I,wplua*:T,s-*:T'
journalctl --user -xb -f -u wireplumber
# The service may not have the DBUS session address...
#+end_src
...
#!/usr/bin/env wpexec
-- -*- mode: lua-ts -*-
-- Plugin.find() is used for Lua scripts registered through wireplumber.conf
        Core.require_api(
            "api.notifications",
            function(notify) -- this was needed in the closure
                if notify ~= nil then
                    notify:call ("send", I18n.gettext("Audio was auto-muted"),
                                 I18n.gettext("Active playback device was disconnected"))
                else
                    print("its nil")
                end
...
I don't think it's D-Bus, since org.pipewire.Telephony is registered (see
[[https://gkiagia.gr/2025-02-20-pipewire-telephony/][Introducing Bluetooth Telephony Support in Pipewire]])
* Pulseaudio
