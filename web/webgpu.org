...
#+begin_quote
This should be split out into =~/Work= using =try= or something
#+end_quote

After running =nix flake update=, run:

#+begin_src shell
chrome=google-chrome-dev # or google-chrome{,-beta}

# TODO: config cache for subs/keys
# TODO: reexport the packages so this works (mostly caches on local system)
NIXPKGS_ALLOW_UNFREE=1 nix run .#$chrome --impure
#+end_src


#+begin_src nix :tangle flake.nix
{
  description = "An isolated build of chrome";
  ...
  inputs.nixpkgs.url = "https://flakehub.com/f/DeterminateSystems/nixpkgs-weekly/0.1.906997";
